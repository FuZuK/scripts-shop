...
$title = 'Журнал';
include(HEAD);
$cr = $db -> res("SELECT COUNT(*) FROM `journal` WHERE `read` = ? AND `id_user` = ?", array(1, $u -> id));
$navi = new navi($cr, '?');
if (!$cr)doc::listEmpty("Нет оповещений");
$q = $db -> q("SELECT * FROM `journal` WHERE `read` = ? AND `id_user` = ? ORDER BY `time` DESC LIMIT {$navi -> start}, {$set -> results_on_page}", array(1, $u -> id));
while ($post = $q -> fetch()) {
