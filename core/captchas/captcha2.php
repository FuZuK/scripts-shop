...
for ($i = 4; $i <= $img_x; $i += 10) {
	$line_color = imageColorAllocateAlpha($img, rand(0, 100), rand(0, 100), rand(0, 100), 100);
	imageLine($img, $i, 0, $i, $img_y, $line_color);
}

// рисуем вертикальные линии
for ($i = 4; $i <= $img_y; $i += 10) {
	$line_color = imageColorAllocateAlpha($img, rand(0, 100), rand(0, 100), rand(0, 100), 100);
	imageLine($img, 0, $i, $img_x, $i, $line_color);
}
